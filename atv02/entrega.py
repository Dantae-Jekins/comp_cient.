...
if __name__ == "__main__":
    ...
    try: file_altu = open("altura.txt", "r");
    except IOError as e:
        print("Erro open()", e );
        file_altu = None;
    
    try: file_anos = open("anos.txt", "r");
    except IOError as e:
        print("Erro open()", e);
        file_anos = None;   
    ## </tratamento de erro>
    ...
    if ( file_altu == None or file_anos == None):
        ...
        while 1:
            ...
            ## filtração dos dados 
            ## eu não vou me dar o trabalho de tratar erros em caso de tipo inválido
            ## pelo fato do tratamento de erro no python ser HORRÍVEL
            ano = float(line_anos);
        ...
        ## conclusão dos dados
        soma = np.sum(altu_array);
        media = soma / np.size(altu_array);
